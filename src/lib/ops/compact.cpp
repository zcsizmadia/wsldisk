...
void CompactOperation::plan_guest_steps(const model::Distro& distro, Plan& plan) {
    ...
    was_running_ = running.has_value() && std::ranges::any_of(*running, [&distro](const std::string& name) {
                       return distro.find_matches(name);
                   });
...
Status CompactOperation::release_disk(const model::Distro& distro, ProgressSink& progress) {
    if (options_.shutdown) {
        // Everything, because that is the only thing that works: the utility VM
        // holds every attached disk for as long as any distribution runs (D9).
        return host_->shutdown();
    }

    if (const Status stopped = host_->terminate(distro.name); !stopped.has_value()) {
        return stopped;
    }
    ...
    return fail(ErrorCode::DistroBusy, std::format("{} is {}", path_.string(), who),
...
void CompactOperation::restart_guest(const model::Distro& distro, ProgressSink& progress, Report& report,
    ...
    // `/bin/true` does nothing in the guest, which is the point: starting it is
