...
2. **lint (clang-format, clang-tidy)** — the same two tools `scripts/lint.ps1` runs locally, against the pinned LLVM and a `compile_commands.json`. Runs on every pull request. It was main-only for a while on the argument that the local script is equivalent; that argument did not survive a clang-tidy slip leaving `main` red across four merges ([#96](https://github.com/zcsizmadia/wsldisk/issues/96)). The real problem was that one clang-tidy process was handling all 38 files on one core, so a full pass took a quarter of an hour and became a check people skipped. Both CI and the local script now run one process per file across the cores: about three and a half minutes locally on twenty of them.
...
> **CodeQL is the one still off pull requests.** `lint-native` and `asan` were
> gated with it, for speed while the platform layer was being built out quickly.
> That cost more than it saved -- a check nobody is made to run is a check
> nobody runs, and `main` went red for four merges because of it -- so both are
> back, the slow one having turned out to be slow for a fixable reason.
>
> CodeQL stays on `main` and the weekly schedule: seven minutes, and it reasons
> about whole-program dataflow rather than a diff, so what it would say on a
> branch it says on `main` a few minutes later. Putting it back is one edit --
> restore the `pull_request: branches: [main]` trigger in `codeql.yml` and set
> `cancel-in-progress` to `${{ github.event_name == 'pull_request' }}` so a
> superseded run stops instead of finishing.
